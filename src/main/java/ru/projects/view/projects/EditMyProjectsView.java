...
@Route(value = "my-projects/:projectID?/:action?(edit)", layout = MainLayout.class)
@RolesAllowed(value = {"ROLE_PM"})
@Menu(order = 4, icon = "line-awesome/svg/project-diagram-solid.svg")
@Slf4j
public class EditMyProjectsView extends Div implements BeforeEnterObserver {

    private static final String PROJECT_ID = "projectID";
    private static final String PROJECT_EDIT_ROUTE_TEMPLATE = "my-projects/%s/edit";
    ...
    public EditMyProjectsView(ProjectService projectService, EmployeeService employeeService) {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        if (projectId.isPresent()) {
            ...
            if (projectFromBackend.isPresent()) {
                ...
                Notification.show(String.format("The requested project was not found, ID = %s", projectId.get()),
                        3000, Position.BOTTOM_START);
                refreshGrid();
                event.forwardTo(EditMyProjectsView.class);
            }
        }
    }
    ...
    private void configureGrid() {
        ...
        grid.asSingleSelect().addValueChangeListener(event -> {
            if (event.getValue() != null) {
                ...
                clearForm();
                UI.getCurrent().navigate(EditMyProjectsView.class);
            }
        });
    }
